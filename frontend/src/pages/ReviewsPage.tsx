...
export function ReviewsPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#1a1816] text-gray-200 overflow-hidden">
      ...
      <ChatCommandBar
        commands={[
          { label: 'Yanıt taslakları hazırla', message: 'tüm olumsuz yorumlara yanıt taslakları hazırla' },
          { label: 'Yorum sentezi', message: `${product?.product_name ?? 'ürünüm'} yorumlarını sentezle, ortak temaları ve acil aksiyon noktalarını çıkar` },
          { label: 'Negatif analizi', message: `${product?.product_name ?? 'ürünüm'} için negatif yorumların kök nedenlerini analiz et` },
          { label: '5 yıldız stratejisi', message: `${product?.product_name ?? 'ürünüm'} için 5 yıldızlı yorum sayısını artıracak strateji öner` },
        ]}
      />
