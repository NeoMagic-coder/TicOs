...
const CommerceControlPage = ({ navigate }: { navigate?: (page: string) => void }) => {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const res = await fetch(url, { headers: backendHeaders(), credentials: 'include' });
      ...
      pushToast({ kind: 'error', title: 'Mağaza kontrolü yüklenemedi', body: message });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setSnapshot(await res.json());
      setLoadError('');
      pushToast({ kind: 'success', title: 'Modül taraması tamamlandı' });
    } catch {
      pushToast({ kind: 'error', title: 'Tarama başarısız' });
    } finally {
      setScanning(false);
    }
  };
    ...
    try {
      ...
      if (data.approval_id) {
        ...
        pushToast({
          kind: 'success',
          title: 'Karar alındı',
          body: data.decision_status,
        });
      }
    } catch {
      pushToast({ kind: 'error', title: 'Aksiyon önerilemedi' });
    }
  };
  ...
  const content = (
    <>
      {loading && !snapshot ? (
        <p className={EASY_MODE ? 'easy-empty' : 'page__sub'}>Yükleniyor…</p>
      ) : loadError && !snapshot ? (
        <div className={EASY_MODE ? 'easy-empty easy-empty--err' : 'page__sub'}>
          <p>Veri alınamadı: {loadError}</p>
          <button type="button" className={`${EASY_MODE ? 'easy-btn easy-btn--ghost' : 'btn btn--ghost'}`} onClick={() => void load(true)}>
            Tekrar dene
          </button>
        </div>
      ) : (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
              ...
              <span
                className="badge"
                style={{
                  background: STATUS_COLORS[overall],
                  color: '#000',
                  padding: '4px 10px',
                  borderRadius: 6,
                  fontSize: 12,
  ...
  if (EASY_MODE) {
    return (
      <EasyPageShell
        ...
        subtitle="AI mağaza sağlık ve risk özeti"
  ...
  return <div className="page commerce-control">{content}</div>;
};
