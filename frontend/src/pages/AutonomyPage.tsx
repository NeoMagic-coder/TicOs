...
const TABS: { id: Tab; label: string; icon: any; agentId: string; accent: string }[] = [
  { id: 'negotiation',     label: 'Müzakere (Simülasyon)',      icon: Handshake,  agentId: 'negotiation_agent',        accent: '#0ea5e9' },
  { id: 'logistics',       label: 'Lojistik (Simülasyon)',      icon: Truck,      agentId: 'logistics_agent',          accent: '#14b8a6' },
  { id: 'dynamic_pricing', label: 'Dinamik Fiyat (Simülasyon)', icon: TrendingUp, agentId: 'dynamic_pricing_agent',    accent: '#f59e0b' },
...
export function AutonomyPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      {tab !== 'decisions' && (
        <div className="bg-amber-500/5 border border-amber-500/30 rounded-xl px-4 py-3 text-xs text-amber-200/80">
          ⚙️ <span className="font-semibold">Simülasyon paneli:</span> Bu hesaplama tarayıcıda yapılır; gerçek bir agent çağırmaz, geçek ticari işlem tetiklemez.
        </div>
      )}
