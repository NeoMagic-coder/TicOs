...
import { Activity, Zap, CheckCircle, Clock, ArrowLeft, BarChart2, Play, RefreshCw } from 'lucide-react';
...
interface SchedulerJob {
  ...
  trigger: string;
  last_run: { started_at: string; finished_at: string; status: 'ok' | 'failed'; tools_used?: number; summary?: string; error?: string } | null;
}
...
function useScheduler() {
  ...
  const refresh = useCallback(async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/v1/scheduler`, { signal: AbortSignal.timeout(4000) });
      if (!res.ok) return;
  ...
  const trigger = useCallback(async (jobId: string) => {
    setTriggering(jobId);
    try {
      await fetch(`${BASE_URL}/api/v1/scheduler/${encodeURIComponent(jobId)}/run`, { method: 'POST' });
      await refresh();
    } finally {
      setTriggering(null);
    }
  ...
  return { jobs, running, triggering, refresh, trigger };
}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-white flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${scheduler.running ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                {scheduler.running ? 'çalışıyor' : 'durduruldu'}
              </span>
            </h2>
            <button
              onClick={() => void scheduler.refresh()}
          ...
          <div className="space-y-3">
            {scheduler.jobs.map((job) => {
              const isTrigering = scheduler.triggering === job.id;
              const lastRun = job.last_run;
              return (
                <div key={job.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-800/50 border border-gray-800">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-xs font-medium text-white font-mono truncate">{job.id}</p>
                      {lastRun && (
                        <span className={`px-1.5 py-0.5 rounded text-[9px] font-semibold ${lastRun.status === 'ok' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                          {lastRun.status === 'ok' ? `✓ ${lastRun.tools_used ?? 0} tool` : '✗ hata'}
                        </span>
                      )}
                    </div>
                    <p className="text-[10px] text-gray-500 mt-0.5 truncate">{job.trigger}</p>
                    {lastRun?.summary && (
                      <p className="text-[10px] text-gray-400 mt-1 line-clamp-1">{lastRun.summary}</p>
                    )}
                    {lastRun?.error && (
                      <p className="text-[10px] text-red-400 mt-1 line-clamp-1">{lastRun.error}</p>
                    )}
                    {job.next_run_time && (
                      <p className="text-[10px] text-gray-600 mt-0.5">
                        Sonraki: {new Date(job.next_run_time).toLocaleString('tr-TR')}
                      </p>
                    )}
                  </div>
                  <button
                    onClick={() => void scheduler.trigger(job.id)}
                    disabled={isTrigering || !scheduler.running}
                    className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-40 bg-indigo-600/20 hover:bg-indigo-600/35 border border-indigo-500/40 text-indigo-300 disabled:cursor-not-allowed"
                    title={!scheduler.running ? 'Scheduler çalışmıyor' : 'Şimdi çalıştır'}
                  >
                    {isTrigering ? <RefreshCw size={11} className="animate-spin" /> : <Play size={11} />}
                    {isTrigering ? 'Tetikleniyor' : 'Şimdi Çalıştır'}
                  </button>
                </div>
              );
            })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filteredAgents.map((agent) => {
          ...
          >
            ...
            {agent.stats.tasks_completed_today === 0 && agent.stats.success_rate === 0 && agent.stats.avg_confidence === 0 ? (
              <div className="mt-4 px-2.5 py-2 rounded-md bg-gray-800/30 border border-gray-800 text-[10.5px] text-gray-500 text-center">
                Henüz görev verisi yok — istatistikler ilk gerçek görevden sonra dolar
              </div>
            ) : (
