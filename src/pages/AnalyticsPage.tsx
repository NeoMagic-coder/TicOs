...
export function AnalyticsPage() {
  ...
  const agentProductivity = agents.map((a) => ({
    ...
    görevler: a.stats.tasks_completed_today,
    başarı: Math.round(a.stats.success_rate * 100),
  }));

  const categoryData = tools.reduce((acc, t) => {
    const existing = acc.find((c) => c.name === t.category);
    if (existing) {
      existing.value += t.stats.total_calls;
    } else {
      acc.push({ name: t.category, value: t.stats.total_calls });
    }
    return acc;
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {tools.sort((a, b) => b.stats.total_calls - a.stats.total_calls).slice(0, 10).map((t) => (
                <tr key={t.tool_id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="py-2 px-3 text-right text-gray-300">{t.stats.total_calls}</td>
                  <td className="py-2 px-3 text-right text-gray-300">{(t.stats.success_rate * 100).toFixed(0)}%</td>
                  <td className="py-2 px-3 text-right text-gray-300">{t.stats.avg_duration_ms}ms</td>
                  <td className="py-2 px-3 text-right text-gray-300">${t.stats.total_cost_usd.toFixed(3)}</td>
                </tr>
              ))}
            </tbody>
          </table>
