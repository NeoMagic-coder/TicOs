...
import {
  ...
  ChevronLeft, FileText, Menu, Rocket, Star, Mic2, Palette, Mail, DollarSign, RotateCcw, Brain
} from 'lucide-react';
...
export function Sidebar() {
  const { currentPage, setCurrentPage, sidebarOpen, toggleSidebar, approvals, onboardedProduct, resetOnboarding, agents, tools } = useStore();
    ...
    <aside className={`fixed left-0 top-0 h-full bg-gray-900 border-r border-gray-800 z-40 transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-64' : 'w-16'}`}>
      ...
      {sidebarOpen && (
        <div className="p-3 border-t border-gray-800 shrink-0">
          <button onClick={resetOnboarding} className="w-full text-[10px] text-gray-500 hover:text-yellow-400 flex items-center justify-center gap-1 py-1 mb-2">
